...
gulp.task('images', () => src(path.src.img)
  .pipe(
    webp({
      quality: 70,
    })
      ...
      if (items) {
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          if (c_fontname != fontname) {
            fs.appendFile(sourceFolder + '/scss/base/fonts.scss', '@include font("' + fontname + '", "' + fontname + '", "400", "normal");\r\n', cb);
          }
          c_fontname = fontname;
        }
      }
...
gulp.task('build', gulp.series('clean', gulp.parallel('scripts', 'scripts:vendor', css, html, 'images', fonts, 'svgSprite')));
